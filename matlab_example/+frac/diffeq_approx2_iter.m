function [ v, fracp ] = diffeq_approx2_iter(fn, fracp, dt)
  ...
  v = fn;
  for j = 1:fracp.N
      v = v - fracp.delta * coeff(j) * coeff(j) * fracp.Q(j,:);
  end
  ...
  fracp.f_prev = v;
  for j = 1:fracp.N
    fracp.Q(j,:) = coeff(j) * coeff(j) * fracp.Q(j,:) + fracp.beta(j) * coeff(j) * df; % updating the prony loss terms ...
  end
end