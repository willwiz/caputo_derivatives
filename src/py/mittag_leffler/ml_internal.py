...
def LTInversion(t,lamda,alpha,beta,gama,log_epsilon):
    ...
    p = gama*np.ones((J1,), dtype=np.float64)
    p[0] = max(0,-2*(alpha*gama-beta+1))
    q = gama*np.ones((J1,), dtype=np.float64)
    q[-1] = np.inf
    ...
    mu_vett = np.ones((JJ1+1,), dtype=np.float64)*np.inf
    N_vett = np.ones((JJ1+1,), dtype=np.float64)*np.inf
    h_vett = np.ones((JJ1+1,), dtype=np.float64)*np.inf
