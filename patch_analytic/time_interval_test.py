...
from src.caputoD import *
from src.AnalyticalSolution import *
from src.ProgressBar import progress_bar
from src.InputOutput import write_array
...
def main(args=None):
  ...
  with futures.ProcessPoolExecutor(args.nc) as executor:
    ...
    e_list  = [1, 2, 3, 4, 5]
    t_list  = [2.0, 8.0, 32.0, 128.0, 512.0]
    tests   = [
      frac_t_anal,
      diff_t_anal,
      diff_frac_t_anal,
    ...
    bar = progress_bar("Running Tf tests", len(a_list)*len(n_list)*len(d_list)*len(e_list)*len(t_list)*len(tests))
    for fc in tests:
      for d in d_list:
        for a in a_list:
          for nk in n_list:
            future_to_time = {}
            fout = os.path.join("data_tf", f"{fc.__name__}_{int(100*d)}_{int(100*a)}_{nk}.txt")
            if not(args.cr) and os.path.exists(fout):
              for _ in range(len(e_list)*len(t_list)):
                bar.next()
              continue
            norm_arr = np.zeros((len(e_list),len(t_list)))
